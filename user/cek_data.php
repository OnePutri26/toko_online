            ...
            <table class="table">
                ...
                <tbody>
                    ...
                    foreach (@$_SESSION['cart'] as $key => $value) : 
                        ...
                        include '../connection.php';
                        $foto = mysqli_query($conn, 'SELECT * FROM barang WHERE id_barang = "'.$value['id_barang'].'" ');
                        $dt_foto = mysqli_fetch_array($foto);
                        ?>
                    <tr>
                        <td><?=($key+1)?></td>
                        <td><img src="./assets/<?=$dt_foto['foto']?>" class="img-fluid rounded-start" alt="..." height="100px"></td>
                        <td><?=$value['nama_barang']?></td>
                        <td> <?php echo $value["harga"]; ?></td>
                        <td><?=$value['qty']?></td>
                        <td> <?php echo 55555;?></td>
                    </tr>
                    <?php endforeach;?>
                </tbody>
            </table>
